 ...
 *
 * @brief Implementation of the Class CrisisAlgorithmNamespace::CrisisAlgorithm
 *
...
CrisisAlgorithm::CrisisAlgorithm(int numberOfCities,
        const vector<pair<int, int> > *connections,
        const vector<pair<int, int> > *toCut, int capitol)
        : m_numberOfCities(numberOfCities), m_capitol(capitol), m_cities(0L),
                m_months(0), m_toCut(toCut), m_result(numberOfCities),
                m_executed(false)
{
    if (m_capitol > m_numberOfCities)
        throw InvalidArgumentsException("m_capitol > m_numberOfCities");
    ...
    for (int i = 0; i < connections->size(); ++i) {
        m_cities[(*connections)[i].first].addUnusedConnection(
                &m_cities[(*connections)[i].second]);
        m_cities[(*connections)[i].second].addUnusedConnection(
                &m_cities[(*connections)[i].first]);
    }
}
...
const CrisisAlgorithmResult& CrisisAlgorithm::execute()
{
    ...
    vector<int> notConnected = m_connectedCities.makeTree(m_capitol, m_cities,
            m_numberOfCities);
    ...
    for (vector<pair<int, int> >::const_iterator it = m_toCut->begin();
        ...
        if (m_unconnectedCities[it->first] != 0L) {
            if (m_unconnectedCities[it->first]->removeUnusedConnectionWith(
                ...
                if (m_unconnectedCities[it->second] != 0) {
                    if (m_unconnectedCities[it->second]
                            ->removeUnusedConnectionWith(it->first)) {
                        continue;
                    } else {
                        throw InvalidArgumentsException(
                                "Algorithm problem here should be not false");
                    } //removeUnused
                } else {
                    throw InvalidArgumentsException(
                            "Only one end of unused connection in unconnected");
                }

            } else {
                throw InvalidArgumentsException(
                        "No such connection from unused");
            }
        }

        ///////////////////////////////

        notConnected = m_connectedCities.removeConnection(it->first,
                it->second);
        ...
        if (m_connectedCities.isRootOnly())
            break;
    }
