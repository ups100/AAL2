...
{
	if(m_capitol == -1) throw InvalidArgumentsException("Case not generated yet");

	ofstream out;
	out.open(fileName.c_str());

	if(!out.is_open()) throw InvalidArgumentsException("Unable to open this file");

	out<<m_nmbOfCities<<endl<<endl;

	for(int i = 0; i < m_nmbOfCities*2; ++i){
		out<<m_connections[i].first<< "\t"<<m_connections[i].second<<endl;
	}
