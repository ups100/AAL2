/**
 * @file CrisisAlgorithmChecker.cpp
 *
...
CrisisAlgorithmChecker::CrisisAlgorithmChecker(int numberOfCities,
        const vector<pair<int, int> > *connections,
        const vector<pair<int, int> > *toCut, int capitol)
        : m_numberOfCities(numberOfCities), m_capitol(capitol), m_toCut(toCut),
                m_result(numberOfCities), m_executed(false)
{
    ...
    for (int i = 0; i < connections->size(); ++i) {
        boost::tie(edge, exist) = boost::edge(
                m_verticles[connections->at(i).first],
                m_verticles[connections->at(i).second], m_graph);

        if (exist) {
            m_graph[edge].m_isDouble = true;
        } else {
            boost::tie(edge, ok) = boost::add_edge(
                    m_verticles[connections->at(i).first],
                    m_verticles[connections->at(i).second], m_graph);

            if (!ok)
                throw InvalidArgumentsException(
                        "Some troubles while edge adding");
        }
